...
from utils.LLM_Tool import gpt_4o, text_embedding_3_large, gpt_4o_for_summary
...
def _process_single_slide(args):
    ...
    try:
        ...
        print(f"✅ 成功為 {file_name} 第 {slide_num} 張幻燈片生成摘要")
        return summary, slide_identifier, slide_num
    except Exception as e:
        print(f"❌ 處理 {file_name} 第 {slide_num} 張幻燈片時出錯: {e}")
        return None, None, slide_num

# 新增的 PPTX 幻燈片摘要生成函數
def generate_pptx_slide_summaries(file_path: str, file_name: str, slide_image_paths: List[Tuple[int, str]], max_workers: int = 4) -> Tuple[List[str], List[str]]:
    """
    為 PPTX 每一張幻燈片生成詳細摘要
    ...
    if not slide_image_paths:
        print(f"❌ 未找到 {file_name} 的幻燈片圖片，無法生成摘要")
        return [], []
    ...
    # 準備工作參數
    tasks = [(llm, slide_num, img_path, file_name) for slide_num, img_path in slide_image_paths]
    
    # 設定最大工作線程數，不超過幻燈片數量
    actual_max_workers = min(max_workers, len(slide_image_paths))
    print(f"🚀 開始並行處理 {file_name} 的 {len(slide_image_paths)} 張幻燈片，使用 {actual_max_workers} 個並行工作線程")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=actual_max_workers) as executor:
        # 提交所有任務
        future_to_slide = {executor.submit(_process_single_slide, task): task[1] for task in tasks}
        ...
        for future in concurrent.futures.as_completed(future_to_slide):
            ...
            try:
                ...
                print(f"❌ 獲取 {file_name} 第 {slide_num} 張幻燈片的摘要結果時出錯: {e}")
                failed_slides += 1
    
    print(f"📊 並行處理完成：成功為 {processed_slides}/{len(slide_image_paths)} 張幻燈片生成了摘要，失敗 {failed_slides} 張")
    ...
    if len(slide_summaries) > 0:
        ...
        # 按幻燈片編號排序
        sorted_results = sorted(zip(slide_summaries, slide_identifiers, slide_numbers), key=lambda x: x[2])
        # 解包排序後的結果
...
def _process_single_page(args):
    ...
    try:
        ...
        print(f"✅ 成功為 {file_name} 第 {page_num} 頁生成摘要")
        return summary, page_identifier, page_num
    except Exception as e:
        print(f"❌ 處理 {file_name} 第 {page_num} 頁時出錯: {e}")
        return None, None, page_num

# 新增的 PDF 頁面摘要生成函數 (並行版本)
def generate_pdf_page_summaries(file_path: str, file_name: str, page_image_paths: List[Tuple[int, str]], max_workers: int = 6) -> Tuple[List[str], List[str]]:
    """
    ...
    if not page_image_paths:
        print(f"❌ 未找到 {file_name} 的頁面圖片，無法生成摘要")
        return [], []
    ...
    # 準備工作參數
    tasks = [(llm, page_num, img_path, file_name) for page_num, img_path in page_image_paths]
    
    # 設定最大工作線程數，不超過頁面數量
    actual_max_workers = min(max_workers, len(page_image_paths))
    print(f"🚀 開始並行處理 {file_name} 的 {len(page_image_paths)} 頁，使用 {actual_max_workers} 個並行工作線程")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=actual_max_workers) as executor:
        # 提交所有任務
        future_to_page = {executor.submit(_process_single_page, task): task[1] for task in tasks}
        ...
        for future in concurrent.futures.as_completed(future_to_page):
            ...
            try:
                ...
                print(f"❌ 獲取 {file_name} 第 {page_num} 頁的摘要結果時出錯: {e}")
                failed_pages += 1
    
    print(f"📊 並行處理完成：成功為 {processed_pages}/{len(page_image_paths)} 頁生成了摘要，失敗 {failed_pages} 頁")
    ...
    if len(page_summaries) > 0:
        ...
        # 按頁碼排序
        sorted_results = sorted(zip(page_summaries, page_identifiers, page_numbers), key=lambda x: x[2])
        # 解包排序後的結果
...
def get_image_hash(image_path):
    ...
    try:
        ...
        print(f"計算圖片哈希時出錯 {image_path}: {e}")
        return None
...
def extract_image_paths(figures_dir: str, filter_images=True) -> list:
    ...
    if not os.path.exists(figures_dir):
        print(f"⚠️ 圖片目錄不存在: {figures_dir}")
        return []
    
    # 獲取所有子目錄
    subdirs = [d for d in os.listdir(figures_dir) if os.path.isdir(os.path.join(figures_dir, d))]
    ...
    # 如果沒有找到圖片，記錄警告
    if not image_paths:
        print(f"⚠️ 在 {figures_dir} 及其子目錄中未找到任何圖片")
        return []
    
    # 過濾無意義的圖片
    if filter_images:
        print(f"🔍 開始過濾圖片，原始圖片數量: {len(image_paths)}")
        ...
        print(f"🔍 過濾完成，過濾後圖片數量: {len(filtered_paths)}/{len(image_paths)}")
        return filtered_paths
    
    return image_paths

def generate_img_summaries(figures_dir: str, filter_images=True) -> Tuple[List[str], List[str]]:
    """
    為圖片生成摘要
    figures_dir: 圖片目錄路徑
    filter_images: 是否過濾無意義的圖片
    ...
    for img_path in image_paths:
        ...
        try:
            ...
            # 準備消息
            system_message = {"role": "system", "content": "用繁體中文回答和使用直覺簡明的方式說明圖片內容：請描述這張圖片中有什麼，盡量細節到位。回答要全部都只有一段話，不要使用條列式或是項目符號。"}
            user_content = [
                {
                    "type": "image_url",
                    "image_url": {"url": f"data:image/jpeg;base64,{img_base64}"}
            ...
            print(f"✅ 成功為圖片 {img_relative_path} 生成摘要")
        except Exception as e:
            print(f"❌ 處理圖片 {img_path} 時出錯: {e}")
    
    print(f"📊 共為 {len(img_summaries)}/{len(image_paths)} 張圖片生成了摘要")
